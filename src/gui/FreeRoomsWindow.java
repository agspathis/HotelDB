 ...
 * @author 
 */
public class FreeRoomsWindow extends javax.swing.JFrame {
     ...
     */
    public FreeRoomsWindow(JTextArea textArea) {
        initComponents();
    ...
    private void findFreeRooms(){
    	if(fromTF.getText().length()!=10 ||
    			toTF.getText().length()!=10){
    	...
    	String FREEROOMQUERY = 
        		...
        		"(SELECT Room_idRoom " +
        		"FROM Rental " +
        		"WHERE (date(arrivalDate)>'"+fromTF.getText()+"' AND date(arrivalDate)<'"+toTF.getText()+"') OR " +
        				"(date(arrivalDate)<'"+fromTF.getText()+"' AND date(departureDate)>'"+fromTF.getText()+"'))";
        		
        		
        
    	
		DatabaseSelect freeRoomSelect = new DatabaseSelect(textArea, FREEROOMQUERY);
		if(freeRoomSelect.getConnectionState()==true&&
				freeRoomSelect.execute()){
			textArea.append(FREEROOMQUERY+"\n");
    	}else{
