...
public class ReservationWindow extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        offerList.setModel(new javax.swing.AbstractListModel() {
            String[] strings = { "Early Booking", "Child", "Dog", "Tour1", "Extra" };
    ...
    private void book(){
    	...
    	if(idTF.getText().length()==0||fNTF.getText().length()==0||
    			lNTF.getText().length()==0||roomTF.getText().length()==0||
    ...
    private boolean customerExist(){
    	...
    	DatabaseGetSelect customerExist = new DatabaseGetSelect(textArea, CUSTOMEREXISTQUERY);
    ...
    private boolean roomFree(){
    	String FREEROOMQUERY = 
        		...
        		"(SELECT Room_idRoom " +
        		"FROM Rental " +
        		"WHERE (date(arrivalDate)>'"+aDTF.getText()+"' AND date(arrivalDate)<'"+dDTF.getText()+"') OR " +
        				"(date(arrivalDate)<'"+aDTF.getText()+"' AND date(departureDate)>'"+aDTF.getText()+"'))";
    	
    	DatabaseGetSelect freeRoom = new DatabaseGetSelect(textArea, FREEROOMQUERY);
    	
    	ResultSet rs = freeRoom.getResult();
    	if(rs!=null){
    		try {
    			if(!rs.next()){
    				return false;
    ...
    private boolean insertCustomer(){
    	String INSERTCUSTOMER = 
    			"INSERT INTO Customer(`idCustomer`, `lastName`, `firstName`, `city`, `country`, `email`)" +
    			"VALUES ('"+idTF.getText()+"', '" +
    						lNTF.getText()+"', '" +
    						fNTF.getText()+"', '" +
    						cityTF.getText()+"', '" +
    	...
    	DatabaseInsert customerInsert  = new DatabaseInsert(textArea, INSERTCUSTOMER);
    	
    	if(customerInsert.getConnectionState()&&
    ...
    private boolean makeRental(){
    	...
    	String MAKERENTAL = 
    			"INSERT INTO Rental(`Room_idRoom`, `Customer_idCustomer`, `arrivalDate`," +
    			" `departureDate`, `dayPrice`, `checkIn`, `checkOut`, `payed`) " +
    			"VALUES ('"+roomTF.getText()+"', '" +
    						idTF.getText()+"', '" +
    						aDTF.getText()+" 12:00:00', '" +
    						dDTF.getText()+" 12:00:00', " +
    						priceTF.getText()+", " +
    						isCurrentDate()+", 0, 0)";
    	...
    	if(makeRental.getConnectionState()==true&&
    			makeRental.execute()){
    		return true;
    	}else{
    		return false;
    ...
    private void makeOffer(){
    	...
    	if(indices.length==0){
    		...
    		for(int i : indices){
    			System.out.println(offerList.getModel().getElementAt(i));
    		}
    	}
    }
